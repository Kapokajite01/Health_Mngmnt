...
window.onclick = function(event) {
    if (event.target == modal) {
        ...
        <div class="container">
            <h4 class="page-head-line"><strong><I>DELETE STATUS SCORING DEFINITION</I></strong></h4><HR>
 </div>  
...
$mydelet = " DELETE FROM  status_customization  WHERE 	stastus_id = '$ID ' ";
if ($conn->query($mydelet) === TRUE) {
echo "<script>alert('Successfully Deleted'); window.location='status_custom'</script>";
} else {
...
echo "<script>alert('Deleted Canceled'); window.location='status_custom'</script>";
