...
<hr>				<form method="post" action="variation_consumable1.php">
			
			<h4 align="center">STOCK VARIATION OF MEDICINES IN HEALTH CENTER&nbsp;&nbsp;&nbsp;&nbsp; <?php echo date('F') ?> <?php echo date('Y') ?></strong>
		...
		<select name="consomablevar" id="consomablevar"  style=" width:200px; height: 25px " required aria-required='true' aria-describedby='name-format'>
...
consomable_mvt.movment,consomable_mvt.bdate FROM consomable_mvt JOIN consommables on consomable_mvt.id= consommables.id WHERE (consomable_mvt.movment='Purchase' or consomable_mvt.movment='OUT') and MONTH(bdate)= MONTH(CURDATE()) GROUP BY bdate Desc ,name_consommable ORDER BY bdate, movment,name_consommable"); 
