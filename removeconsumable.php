...
$rowid = $_GET['rowid'];
$resultret = ("SELECT * FROM temp_consommable WHERE patient_id  = '$numbertest'");
$pat = mysqli_query($conn, $resultret);
while ($row = mysqli_fetch_assoc($pat)) {
$medic = $row['cons_description'];
$qty = $row['quantity'];
$price = $row['price'];
}
if(isset($_POST['deletetest'])){
	$testdelete = mysqli_query($conn,"DELETE FROM temp_consommable WHERE temp_consId='".$rowid."'");
	if($testdelete){
	?>	
...
<form method="post" action=""  onsubmit="return deleteConfirm();">
				...
				<table>
	<table>
	<tr><td><label>Consumable</label> </td><td>&nbsp;&nbsp;<?php echo $medic;?></td></tr>	
	<?php 
	if($qty)
	{
	?>
