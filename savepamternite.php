...
$insert = ("INSERT INTO consomation_consom (id,consultatiobn,medicament,upmedicamnet,qtymedicamnet,consommable,UPcons,Qcons,actemedicale,upacte,qtyacte,examenmedicale,upexamen,qtyexamen,hospitalization,uphospitalizations,qtyhoapitalization,FdtHospitalisation,LdtHospitalisation,ambullance,upambullance,qtyambullance,datecunsuption,user,postedesante)
      VALUES('$pid','$consultation','$medicments[$ih]', '$pricemecamen[$ih]','$qtymedicamen[$ih]','$consomables[$ih]','$pricecosnmble[$ih]','$qtyconsmble[$ih]','$actemedic[$ih]', '$pricemedacte[$ih]','$qtymedacte[$ih]','$ex7','$upex7','$qex7','$hosptlstion[$ih]','$pricehosp[$ih]','$qtyhosp[$ih]','$firstDate[$ih]','$lastDate[$ih]','$amblnce[$ih]','$amtkm[$ih]','$ambukm[$ih]',Now(),'$userid','$postedesante')");
...
$insertAmbul = ("INSERT INTO consomation_consom (id,consultatiobn,ambullance,upambullance,qtyambullance,datecunsuption,user,postedesante)
	...
	if($resultsave or $resultsaveAmb){
...
if($examendifferent){
$INSERTdiferenc = ("INSERT INTO doctor_comments (comment_if,patient_id,type,comment,Doctor,comment_time)VALUES('','$patient','Diagnostic Diferenciel ','$examendifferent','$userid',Now())");
...
echo "<script>window.location='maternite';</script>";
	}

	ELSE{
		if (!empty($consultation) OR ($consultation != '0')) {
	$consult = ("INSERT INTO consomation_consom (id,consultatiobn,ambullance,upambullance,qtyambullance,datecunsuption,user,postedesante)VALUES('$pid','$consultation','$amblnce','$amtkm','$ambukm',Now(),'$userid','$postedesante')");
...
$UPDATEstatus= "UPDATE maternite SET status  = 'Completed' WHERE  	patient_id   ='$patient' and time_reception = '$myndate'"; 
mysqli_query($conn,$UPDATEstatus);
