...
$rowid = $_GET['rowid'];
$resultret = ("SELECT * FROM temp_actemed WHERE patient_id  = '$numbertest'");
$pat = mysqli_query($conn, $resultret);
while ($row = mysqli_fetch_assoc($pat)) {
$medic = $row['acte_description'];
$qty = $row['quantity'];
$price = $row['price'];
}
if(isset($_POST['deletetest'])){
	$testdelete = mysqli_query($conn,"DELETE FROM temp_actemed WHERE tempact_Id='".$rowid."'");
	if($testdelete){
	?>	
