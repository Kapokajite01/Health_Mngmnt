...
<hr/>	
    <div class="tabbed_area">  
       ...
       $result = ("SELECT * FROM prescription")or die(mysql_error());
...
?> 
			...
			$(document).ready(function()
	{
		...
		<?php
		$invNum=  ("SELECT 1+MAX(invoice_id) FROM invoice");
		$invoice=mysqli_fetch_assoc($invNum);
		if($invoice[0]=='')
		{$invoiceNo=10; }
		...
		<form name="myform" onsubmit="return validateForm(this);" action="invoice.php" method="post" >
			<table width="200" height="106" border="0" >	
				...
				echo"<select  class=\"input-small\" name=\"drug_name\" style=\"width:170px\" id=\"drug_name\">";
						 $getpayType=("SELECT drug_name FROM stock");
						 echo"<option>Select Drug</option>";
		 while($pType=mysqli_fetch_assoc($getpayType))
			{
