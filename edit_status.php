...
window.onclick = function(event) {
    if (event.target == modal) {
        ...
        <div class="container">
            <h4 class="page-head-line"><strong><I>MARTIAL STATUS SCORING DEFINITION EDIT</I></strong></h4><HR>
 </div>  
...
<a href="status_custom"class="btn btn-success" style="width:auto; "><i class="glyphicon glyphicon-back"></i>BACK</a>
...
<?php
$ID = $_GET['extr_id'];
  $query="select * from status_customization where stastus_id='$ID'";
  ...
  ?>					
					<form method="post" enctype="multipart/form-data" class="form-horizontal" style="margin-left:250px;">
					  ...
					  <div class="form-group">
						<label for="inputEmail3" class="col-sm-2 control-label"><Strong>Status Name</strong></label>
						<div class="col-sm-4">
						  <input type="text" name="statusname" value="<?php echo $statusname; ?>" class="form-control" id="inputEmail3" required />
						</div>
...
if (isset($_POST['update'])) {
						...
						$score=$_POST['score'];
$statusupdate = " UPDATE status_customization SET 	status_name='$statusname', status_score='$score' WHERE 	stastus_id = '$ID ' ";
if ($conn->query($statusupdate) === TRUE) {
echo "<script>alert('Successfully Updated Martial Status Scoring!'); window.location='status_custom'</script>";
}
