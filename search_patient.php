...
$result = ("SELECT patient.id,patient.names,patient.lname,patient.insurance,patient.category,patient.affiliate_number,patient.gender,patient.dob,patient.affiliate_names,patient.afilliate_lastname,patient.familycode,consomation.consultatiobn,datecunsuption FROM consomation JOIN  patient ON consomation.id=patient.id WHERE affiliate_number  = '$afnumber' group by id,datecunsuption order by patient.id DESC"); 
$resultselect = mysqli_query($conn, $result);
if ($resultselect->num_rows > 0) {
echo "<table class='blueTable'>";
         echo "<tr><TH>nO</TH><th>Names</th><th>affiliatION number</th><th>gender</th>
		 <th>dob</th><th>Date</th><th>Edit</th</tr>";
        while($row = mysqli_fetch_assoc( $resultselect )) {
                $mydate1=date("d-m-Y", strtotime($row['datecunsuption']));
                echo "<tr>";
				echo '<td>' .++$IJ .'</td>';
				...
				<td><a href="edit_record?id=<?php echo $row['id']?>&datecons=<?php echo $row['datecunsuption']?>"><img src="images/update-icon.png" width="24" height="24" border="0" /></a></td>
			<?php
		 } 
        echo "</tr></table>";
}
