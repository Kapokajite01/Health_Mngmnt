...
$actemed = $row['actemedicale'];
$qty = $row['upacte'];
$price = $row['qtyacte'];
}
if(isset($_POST['deletetest'])){
	$testdelete = mysqli_query($conn,"UPDATE consomation_consom SET actemedicale  = '', qtyacte   = '', upacte  = ''   WHERE consid='".$numbertest."'");
	if($testdelete){
	?>	
...
<form method="post" action=""  onsubmit="return deleteConfirm();">
				...
				<table>
	<table>
	<tr><td><label>Medical Act</label> </td><td>&nbsp;&nbsp;<?php echo $actemed;?></td></tr>	
	<?php 
	if($qty)
	{
	?>
