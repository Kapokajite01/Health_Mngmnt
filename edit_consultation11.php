...
$sql="update consomation set consultatiobn = '$prodname' where id='$myid'and datecunsuption='$date'";
...
$id=$_POST['id'];
$date=$_POST['date'];
$result = ("SELECT patient.names,patient.lname,patient.affiliate_number,consomation.consid,consomation.id as pid,consomation.consultatiobn,consomation.datecunsuption 
FROM consomation JOIN patient on patient.id= consomation.id where  consomation.id ='$id' and datecunsuption='$date' GROUP BY affiliate_number"); 
...
echo "<table border='1'>";
        ...
        while($row = mysqli_fetch_assoc( $resultselect )) {
				...
				echo '<td width="">' . $row['consultatiobn'] . '</td>';
				echo '<td>' . $row['datecunsuption'] . '</td>';
$myid=$row['id'];
echo $myid;
				?>

				<td><a href="edit_consultation1?consid=<?php echo $row['consid']?>"><img src="images/update-icon.png" width="24" height="24" border="0" /></a></td>
			<?php
		 } 
