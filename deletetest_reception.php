...
$test = $row['medicament'];
$qty = $row['qtymedicamnet'];
$price = $row['upmedicamnet'];
}
if(isset($_POST['deletetest'])){
	$testdelete = mysqli_query($conn,"UPDATE consomation_consom SET medicament = '',qtymedicamnet = '',upmedicamnet = ''   WHERE consid='".$numbertest."'");
	if($testdelete){
	?>	
...
<form method="post" action=""  onsubmit="return deleteConfirm();">
				...
				<table>
	<table>
	<tr><td><label>Medicine</label> </td><td>&nbsp;&nbsp;<?php echo $test;?></td></tr>	
	<?php 
	if($qty)
	{
	?>
