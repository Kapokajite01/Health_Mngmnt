...
if (isset($_FILES) && (bool) $_FILES) {
  ...
  // Define allowed extensions
    ...
    $allowedExtentsoins = ["pdf", "doc", "docx", "gif", "jpeg","jpg", "png", "rtf", "txt","zip","xlsx","xls","ppt","pptx","csv"];
    ...
    foreach ($_FILES as $name => $file) {
        if (!$file['name'] == "") {
           ...
           if (in_array($ext, $allowedExtentsoins)) {
			...
			echo $i.'  '. $file_name .' - Exention ['.$ext.']<br>';
...
$insertloandetails = "INSERT INTO loan_details(loan_id,loan_tracknumber,loan_account,loan_amount,loan_type,loan_incharge,loan_status,branch,time_application,loan_term_type,loan_term_number,loan_interest,first_payment) 
VALUES('','$tracknumber','$accountnumber','$amount','$loantype','','','$branch',Now(),'$loan_term_type','$loantermduration','$loantermrate','$firstpymt')";
$conn->query($insertloandetails);

//insert into sibblings

for($i=0; $i<$contsibling; $i++){
$siblinglINSERT = "INSERT INTO siblings(sibling_id,loan_id,sibling_type,sibling_firtsname,sibling_lastname,sibling_idnumber,sibling_phone,sibling_adress) VALUES ('','$tracknumber','".addslashes($types[$i])."','".addslashes($fnames[$i])."','".addslashes($lnames[$i])."','".addslashes($sblidnumber[$i])."','".addslashes($siblingtelephone[$i])."','".addslashes($address[$i])."')";
...
for($n=0; $n<$countcoraterals1; $n++){
	...
	$name = $_FILES["files"]["name"][$n];
			$corfile_name[$n] = $randomcor.'_'.$name;
			$corfile_name[$n] = preg_replace('/\s+/', '_', $corfile_name[$n]);
			$corfile_name[$n] = str_replace('+','_', $corfile_name[$n]);
			move_uploaded_file($temp,"files/sdjklnkjbnkbsfs/".$corfile_name[$n]);


}
echo  "Coraterals <br>";
 for($sk = 0; $sk< $countcoraterals; $sk++ ){
			echo ($sk+1).' '. $corfile_name[$sk].'<br>' ;
	$insertincomes = "INSERT INTO coraterals(coraterals_id,loan_id,corateral_type,corateral_value,attacment) 
VALUES('','$tracknumber','".addslashes($typecorateral[$sk])."','".addslashes($corateral[$sk])."','".$corfile_name[$sk]."')";
$conn->query($insertincomes); 
	 
 }

 echo  "Refferences <br>";
 for($po = $countcoraterals; $po< $countcoraterals1; $po++ ){
	 $reffile_name[]= $corfile_name[$po];
			echo ($po+1).' '. $corfile_name[$po].'<br>' ;
	 } 
...
/*
$refndomcor = rand();
for($pp=$countcoraterals; $pp < $countcoraterals; $pp++){
	$tempr[$pp] = $_FILES["files"]["tmp_name"][$pp];
	$namer[$pp] = $_FILES["files"]["name"][$pp];
			$corfile_namer[$pp] = $refndomcor.'_'.$namer[$pp];
			$corfile_namer [$pp]= preg_replace('/\s+/', '_', $corfile_namer[$pp]);
			$corfile_namer[$pp] = str_replace('+','_', $corfile_namer[$pp]);
			echo $corfile_namer[$pp].'<br>';
			move_uploaded_file($tempr[$pp],"files/sdjklnkjbnkbsfs/".$corfile_namer[$pp]);
}*/
/*$insertrefferences = "INSERT INTO applicanr_refference(reff_id,loan_id,reff_name1,reff_address1,reff_telephone1,reff_name2,reff_address2,reff_telephone2,reff_name3,reff_address3,reff_telephone3,attacment1,attachment2,attachment3) 
VALUES('','$tracknumber','$refference1','$refferenceaddres1','$refferencenumber1','$refference2','$refferenceaddres2','$refferencenumber2','$refference3','$refferenceaddres3','$refferencenumber3','$corfile_namer[$pp]','$corfile_namer[$pp]','$corfile_namer[$pp]')";
$conn->query($insertrefferences);
