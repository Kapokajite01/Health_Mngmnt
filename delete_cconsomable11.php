...
$sql="update consomation_upload set up_consomable = '$prodname', up_upcons='$pcost', up_qtycons = '$qty' where up_id='$id'";
($sql);
...
$id=$_POST['id'];
$date=$_POST['date'];
$result = ("SELECT up_pname,up_plname,up_afnumber,up_id,up_pid,up_consomable,up_upcons,up_qtycons,datecunsuption 
FROM consomation_upload where up_consomable!='' AND up_pid ='$id' and datecunsuption='$date'") 
                or die(mysql_error());
echo "<table border='1'>";
        ...
        while($row = mysqli_fetch_assoc( $result )) {
				...
				echo '<td width="">' . $row['up_consomable'] . '</td>';
				echo '<td>' . $row['up_upcons'] . '</td>';
				echo '<td>' . $row['up_qtycons'] . '</td>';
				echo '<td>' . $row['datecunsuption'] . '</td>';

				?>

				<td><a href="edit_cconsumable1?consid=<?php echo $row['up_id']?>"><img src="images/update-icon.png" width="24" height="24" border="0" /></a></td>
				<td><a href="delete_cconsumable1?consid=<?php echo $row['up_id']?>"><img src="images/delete-icon.jpg" width="24" height="24" border="0" /></a></td>
			<?php
		 } 
        // close table>
        echo "</table>";
