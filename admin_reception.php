...
VALUES('','$name','$lname','$tel','Displayed Users Receptions',now())");
	}
...
VALUES('$fname','$lname','$phone','$user','$pas','$rol')");
	$sql=("INSERT INTO logs(id,name,lname,phone,action,time)
VALUES('','$name','$lname','$tel','Added New User Receptionist',now())");	
	
	header("location:http://".$_SERVER['HTTP_HOST'].dirname($_SERVER['PHP_SELF'])."/admin_reception");
}else{
...
<hr/>	
    <div class="tabbed_area">  
		...
		*/
        ...
        $result = ("SELECT * FROM users where role='Receptionist'") 
                or die(mysql_error());
        ...
        while($row = mysqli_fetch_assoc( $result )) {
				...
				echo '<td>' . $row['role'] . '</td>';
				?>
				<td><a href="update_reception?username=<?php echo $row['username']?>"><img src="images/update-icon.png" width="35" height="35" border="0" /></a></td>
				<td><a href="delete_reception?Reception_Id=<?php echo $row['id']?>"><img src="images/delete-icon.jpg" width="35" height="35" border="0" /></a></td>
				<?php
		 } 
        // close table>
        echo "</table>";
?> 
		...
		<?php echo $message;
			  echo $message1;
			  ?>
		<form name="form1"  onsubmit="return validateForm(validation_script.js);" action="admin_reception" method="post" >
