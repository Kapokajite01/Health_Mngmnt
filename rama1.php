...
<input type="button" id="printButton1" onclick="tableToExcel('example', 'W3C Example Table')" value="Export To Excel">
...
$sql = ("SELECT patient.names,patient.lname,patient.insurance,patient.category,patient.affiliate_number,patient.gender,patient.dob,patient.affiliate_names,patient.afilliate_lastname,patient.familycode,patient.pagenumber,patient.affectation,consomation.consultatiobn,consomation.datecunsuption,sum(consomation.upmedicamnet*consomation.qtymedicamnet)as totmedicament,sum(consomation.UPcons*consomation.Qcons) as totconsommables,
 sum(consomation.upacte*consomation.qtyacte) AS totacte,sum(consomation.uphospitalizations*consomation.qtyhoapitalization)AS tot_hosp,sum(consomation.qtyexamen*consomation.upexamen) as totexamens,sum(consomation.upambullance*consomation.qtyambullance) AS amtambul,patient.null_tm,consomation.postedesante,consomation.id as cid
 FROM consomation JOIN  patient ON consomation.id=patient.id WHERE insurance='RAMA/RSSB' and datecunsuption >= '$date' and datecunsuption <= '$date1'  and comments != 'photocopy' GROUP BY affiliate_number,datecunsuption
 ...
 sum(up_upacte*up_qtyacte) AS totacte,sum(up_uphospita*up_qtyhosp)AS tot_hosp,sum(up_upexame*up_qtyexame) as totexamens,sum(up_upambul*up_qtyambul) AS amtambul,null_tm,postedesante,up_id as cid
 FROM  consomation_upload WHERE up_insurance='RAMA/RSSB'  and datecunsuption >= '$date' and datecunsuption <= '$date1' GROUP BY affiliate_number,datecunsuption ORDER BY datecunsuption,pagenumber ");
...
$result1 = ("SELECT patient.names,patient.lname,patient.insurance,patient.category,patient.affiliate_number,patient.gender,patient.dob,patient.affiliate_names,patient.afilliate_lastname,patient.familycode,patient.pagenumber,patient.affectation,consomation.consultatiobn,consomation.datecunsuption,sum(consomation.upmedicamnet*consomation.qtymedicamnet)as totmedicament,sum(consomation.UPcons*consomation.Qcons) as totconsommables,
 ...
 FROM consomation JOIN  patient ON consomation.id=patient.id WHERE insurance='RAMA/RSSB' and postedesante = 'POSTE DE SANTE KABUGA' and datecunsuption >= '$date' and datecunsuption <= '$date1' GROUP BY affiliate_number,datecunsuption
 UNION ALL SELECT up_pname AS names,up_plname AS lname,up_insurance,up_pcateg as category,up_afnumber AS affiliate_number,up_pgender AS gender,up_pdob AS dob,up_afnme AS affiliate_names,up_aflname as afilliate_lastname,up_famcode as familycode,up_pagenumber AS pagenumber,affectation,up_consom_sonsultation AS consultatiobn,datecunsuption,sum(up_cons_upmedic*up_cons_qytmedic)as totmedicament,sum(up_upcons*up_qtycons) as totconsommables,
 ...
 FROM  consomation_upload WHERE up_insurance='RAMA/RSSB' and postedesante = 'POSTE DE SANTE KABUGA' and datecunsuption >= '$date' and datecunsuption <= '$date1'  GROUP BY affiliate_number,datecunsuption ORDER BY datecunsuption ");
      ...
      <table width="595" border="0" cellspacing="0" cellpadding="0" id="example">
        ...
        <tr>
          <td><table cellpadding="" id="example" cellspacing="" border="0" class="altrowstable">
    		...
    		<th>COST FOR MEDI CAL IMA GING</th>
    		<th>COST FOR HOSPI TALI ZATION</th>
			<th>COST FOR PROCI DURE AND MATE RIALS</th>
			<th>COST FOR OTHER CONSU MABLES</th>
			<th>COST FOR MEDI CINES</th>
...
<th><?php echo number_format((float)$total_coopymt+$total_coopymt1,1);?></th>
     ...
     <div><table><tr><td>&nbsp;&nbsp;</td><td>&nbsp;&nbsp;</td><td><strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</strong></td><td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td><td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td><td><STRONG>Verifi&#233;&nbsp;&nbsp;&nbsp; par:<br>NOM TUTILAIRE POST NOM TUTILAIRE</STRONG></td><td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td><td><STRONG>Approuv&#233; par:<br> NYIRANGWABIJE ALEXIA</STRONG></td></tr></table></div>
