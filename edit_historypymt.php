...
window.onclick = function(event) {
    if (event.target == modal) {
        ...
        <div class="container">
            <h4 class="page-head-line"><strong><I>HISTORY PAYMENT SCORING DEFINITION EDIT</I></strong></h4><HR>
 </div>  
...
<?php
$ID = $_GET['extr_id'];
  $query="select * from payment_customization where payment_id='$ID'";
$result = $conn_db->query($query);
  if ($result->num_rows > 0) {
  while($row = $result->fetch_assoc()) {
 $paymnettype = $row['payment_type'];
 $payment= $row['payment_score'];

  }  }?>					
					<form method="post" enctype="multipart/form-data" class="form-horizontal" style="margin-left:250px;">
					  
				  <div class="form-group">
						<label for="inputEmail3" class="col-sm-2 control-label"><Strong>Payment Type</strong></label>
						 <div class="col-sm-4">
						<input type="text" name="paymenttype" value="<?php echo $paymnettype; ?>" class="form-control" id="inputEmail3" required />
						</div>
					  ...
					  <div class="form-group">
						...
						<div class="col-sm-4">
						  <input type="text" name="score" value="<?php echo $payment; ?>" class="form-control" id="inputPassword3"  required />
						</div>
...
if (isset($_POST['update'])) {
						...
						$score=$_POST['score'];
$historyupdate = " UPDATE payment_customization SET 	payment_type='$paymenttype',payment_score='$score' WHERE payment_id = '$ID' ";
if ($conn->query($historyupdate) === TRUE) {
echo "<script>alert('Successfully Payment History Scoring!'); window.location='lastpayment_custom'</script>";
}
