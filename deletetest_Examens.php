...
$examen = $row['examenmedicale'];
$qty = $row['qtyexamen'];
$price = $row['upexamen'];
}
}
else{
$resultret = ("SELECT * FROM consom_labs WHERE lbaconsid = '$numbertest'");
$pat = mysqli_query($conn, $resultret);
while ($row = mysqli_fetch_assoc($pat)) {
$examen = $row['examen'];
$qty = $row['examen_qty'];
$price = $row['examen_price'];
}	
}
if(isset($_POST['deletetest'])){
	if($thisstatu == 'Completed' )
{
$testdelete = mysqli_query($conn,"UPDATE consomation_consom SET examenmedicale   = '', qtyexamen = '', upexamen = ''   WHERE consid='".$numbertest."'");
}
else{
	$testdelete = mysqli_query($conn,"UPDATE consom_labs SET examen   = '', examen_qty = '', examen_price = ''   WHERE  	lbaconsid='".$numbertest."'");
...
<form method="post" action=""  onsubmit="return deleteConfirm();">
				...
				<table>
	<table>
	<tr><td><label>Examen</label> </td><td>&nbsp;&nbsp;<?php echo $examen;?></td></tr>	
	<?php 
	if($qty)
	{
	?>
