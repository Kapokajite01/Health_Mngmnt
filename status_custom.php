...
$score=$_POST['score'];
$quwry = "INSERT INTO status_customization (stastus_id,status_name,status_score)
					 VALUES ('','$statusname','$score')";
IF ($conn->query($quwry) === TRUE) {
						echo "<script>alert('New Martial Status  score added successfully !!!'); window.location='status_custom'</script>";
} else {
...
<h1>DEFINE MARTIAL STATUS SCORE</h1>
<tr><td>Status Name</td><td><input type="text" name="StatusName" class="form-control" id="inputEmail3" placeholder="Status Name" required/></td></tr>		 
		...
		url,'popUpWindow','height=1000,width=1200,position= absolute,left=400,top=10,resizable=yes,scrollbars=yes,toolbar=yes,menubar=no,location=no,directories=no,status=yes')
                      ...
                      <div class="notice-board">
                        <div class="panel panel-default">
                         ...
                         <button class="btn btn-success" onclick="document.getElementById('id01').style.display='block'" style="width:auto; "><i class="glyphicon glyphicon-plus"></i><strong>DEFINE NEW MARTIAL STATUS SCORING</strong></button>
                    ...
                    <hr />
                       ...
                       <table id="myTable" style="background-image: url('user_images/logo1.png'); width:100%; background-color:#ffffff; background-repeat: no-repeat;background-position: center;" >
                                    ...
                                    <tbody>
                                <?php
							$ststusscore= "select * from status_customization ";
							$result = $conn_db->query($ststusscore);
							if ($result->num_rows > 0) {
							while($row = $result->fetch_assoc()) {
							$id=$row['stastus_id'];
							$statusname = $row['status_name'];
							$statusscore = $row['status_score'];
							?>
                              <tr>
						<td><?php echo ++$i; ?></td>
						<td> <?php echo $statusname; ?></td>
						<td><span class="label-primary label label-default"><?php echo $statusscore; ?> %</span></td>
						<td class="center">
							<!--<a class="btn btn-success" href="#">
								<i class="glyphicon glyphicon-zoom-in icon-white"></i>
								View
							</a>-->
							<a href="edit_status<?php echo '?extr_id='.$id; ?>">
							...
							<a class="btn btn-danger" href="delete_status <?php echo '?extr_id='.$id; ?>">
