...
$consommable = $row['consommable'];
$qty = $row['UPcons'];
$price = $row['Qcons'];
}
if(isset($_POST['deletetest'])){
	$testdelete = mysqli_query($conn,"UPDATE consomation_consom SET consommable  = '',Qcons  = '',upmedicamnet = ''   WHERE consid='".$numbertest."'");
	if($testdelete){
	?>	
...
<form method="post" action=""  onsubmit="return deleteConfirm();">
				...
				<table>
	<table>
	<tr><td><label>Consumable</label> </td><td>&nbsp;&nbsp;<?php echo $consommable;?></td></tr>	
	<?php 
	if($qty)
	{
	?>
