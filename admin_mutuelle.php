...
VALUES('','$name','$lname','$tel','Added New User Mutuelle',now())");	
	
	header("location:http://".$_SERVER['HTTP_HOST'].dirname($_SERVER['PHP_SELF'])."/admin_mutuelle");
}else{
...
<hr/>	
    <div class="tabbed_area">  
		...
		*/
        ...
        $result = ("SELECT * FROM users where role= 'Mutuelle'") 
                or die(mysql_error());
        ...
        while($row = mysqli_fetch_assoc( $result )) {
				...
				<td><a href="delete_mutuelle?skeeper_Id=<?php echo $row['id']?>"><img src="images/delete-icon.jpg" width="35" height="35" border="0" /></a></td>
				<?php
		 } 
        // close table>
        echo "</table>";
?> 
		...
		<?php echo $message;
			  echo $message1;
			  ?>
		<form name="form1"  onsubmit="return validateForm(validation_script.js);" action="admin_mutuelle" method="post" >
